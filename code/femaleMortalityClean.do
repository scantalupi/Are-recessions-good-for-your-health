...
*** mortality rate (total and by cause) for females and for age class
...
* Keep observations for only females
...
save "$projectpath/data/raw/clean/femaleUnemploymentRate.dta", replace 
...
save "$projectpath/data/raw/clean/femaleDeaths.dta", replace 
...
save "$projectpath/data/raw/clean/populationFemale.dta", replace 

* Merge the datasets 
clear all

use "$datapath/clean/femaleDeaths.dta"

merge m:1 AID using "$datapath/clean/populationFemale.dta"

* Calculate death rate per 1,000 females 
gen death_rate = (deaths / population) * 1000
label var death_rate "All Causes Female Mortality Rate"

* Save the dataset 
save "$projectpath/data/raw/clean/femaleDeathRate.dta", replace 
...
save "$projectpath/data/raw/clean/femaleDeathsSpecific.dta", replace 



*** Calculate the rate for specific causes of death ***
clear all 

*** Build a dataset to obtain the values of all type of deaths for each age category ***
use "$datapath/clean/femaleDeaths.dta", clear

* Merge the two datasets
use "$datapath/clean/femaleDeathsSpecific.dta"

merge m:1 AID using "$datapath/clean/populationFemale.dta"

* Generate the rates for each specific cause of death
gen malignant_tumors_rate = (malignant_tumors / population) * 1000 
label var malignant_tumors_rate "Malignant Tumors Female Death Rate"

gen ischameic_hearth_rate = (ischameic_hearth / population) * 1000
label var ischameic_hearth_rate "Ischameic Hearth Diseases Female Death Rate"

gen cerebrovascular_rate = (cerebrovascular / population) * 1000
label var cerebrovascular_rate "Cerebrovascular Diseases Female Death Rate"

gen other_hearth_rate = (other_hearth / population) * 1000
label var other_hearth_rate "Other Hearth Diseases Female Death Rate"

gen digestive_system_rate = (digestive_system / population) * 1000
label var digestive_system_rate "Digestive System Diseases Female Death Rate"

gen transportation_accidents_rate = (transportation_accidents / population) * 1000
label var transportation_accidents_rate "Transportation Accidents Female Death Rate"

gen sucidie_rate = (sucidie / population) * 1000
label var sucidie_rate "Sucidie Female Rate"

gen nonmalignant_tumors_rate = (nonmalignant_tumors / population) * 1000 
label var nonmalignant_tumors_rate "Malignant Tumors Female Death Rate"

gen h_c_amenable_rate = (h_c_amenable / population) * 1000
label var h_c_amenable_rate "Health-Care-Amenable Female Death Rate"
...
save "$projectpath/data/raw/clean/femaleDeathsSpecificRate.dta", replace
...
* Save the dataset 
save "$projectpath/data/raw/clean/femaleActivtyRate.dta", replace
...
use "$datapath/clean/femaleDeathsSpecificRate.dta", clear

* Merge with the second dataset using m:1 and AID as the key variable
merge m:1 AID using "$datapath/clean/femaleActivtyRate.dta", nogen

* Save the merged dataset
save "$datapath/clean/merged6.dta", replace


* Load the third dataset
use "$datapath/clean/femaleDeathRate.dta", clear

* Merge with the previously merged dataset using m:1 and AID as the key variable
merge m:1 AID using "$datapath/clean/merged6.dta", nogen

* Save the merged dataset
save "$datapath/clean/merged7.dta", replace


* Load the fourth dataset
use "$datapath/clean/femaleUnemploymentRate.dta", clear

* Merge with the previously merged dataset using m:1 and AID as the key variable
merge m:1 AID using "$datapath/clean/merged7.dta", nogen force
...
save "$datapath/clean/femaleFinal.dta", replace
